...
| 12 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Connect dan query Azure Database for PostgreSQL via SDK | Azure Database for PostgreSQL | d2-02 | `psycopg` + Entra passwordless (token ossrdbms-aad); CRUD berparameter | SRC-002, SRC-028, SRC-072 | Implementation sources verified | Draft |
| 13 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Modelkan schema dan strategi indexing: desain tabel dan pemilihan data type | Schema/index design | d2-02 | Tabel embedding+metadata; `vector(n)` (dimensi wajib utk index); allowlist + CREATE EXTENSION | SRC-002, SRC-028, SRC-071 | Implementation sources verified | Draft |
| 14 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Strategi indexing: optimasi latency query dan pengurangan overhead compute pgvector | pgvector indexing | d2-02 | IVFFlat/HNSW/DiskANN + tuning (lists/probes/ef_search); EXPLAIN ANALYZE; batas 2000 dim; `<#>` utk vektor ternormalisasi | SRC-002, SRC-028, SRC-073 | Implementation sources verified | Draft |
| 15 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Konfigurasi compute, memory, dan storage untuk vector workloads | Sizing Flexible Server | d2-02 | Tier Burstable/GP/Memory Optimized; vCore naik-turun, storage naik saja | SRC-002, SRC-028, SRC-075 | Implementation sources verified | Draft |
| 16 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Vector similarity search: simpan embedding, semantic retrieval, pola RAG dengan metadata filter | pgvector + RAG | d2-02 | WHERE metadata + ORDER BY operator jarak; partial index (klausa persis) & partitioning | SRC-002, SRC-028, SRC-071, SRC-073 | Implementation sources verified | Draft |
| 17 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Optimasi koneksi untuk throughput dan latency | Connection pooling/optimization | d2-02 | PgBouncer built-in port 6432 (transaction mode); persist credential/token | SRC-002, SRC-028, SRC-074, SRC-072 | Implementation sources verified | Draft |
| 18 | 2 | Integrate Azure Managed Redis in AI solutions | Operasi data Azure Managed Redis: caching, expiration, invalidation | Azure Managed Redis | d2-03 | Cache-aside + TTL + invalidation via Python (`redis`+`redis-entraid`, port 10000, Entra ID) | SRC-002, SRC-029, SRC-076, SRC-078 | Implementation sources verified | Draft |
| 19 | 2 | Integrate Azure Managed Redis in AI solutions | Vector indexing untuk similarity search | Vector search di Redis (RediSearch) | d2-03 | Provisioning (RediSearch saat pembuatan + Enterprise clustering + NoEviction); FLAT/HNSW; hybrid search; sintaks FT dari docs tertaut (Needs Review saat eksekusi) | SRC-002, SRC-029, SRC-077 | Implementation sources verified | Draft |
| 20 | 3 | Develop event- and message-based AI solutions | Queue & proses operasi back-end via Service Bus: messages, topics, subscriptions, dead-letter handling | Azure Service Bus | d3-01 | Kirim/terima async passwordless; topic+subscriptions (salinan per subscription); DLQ: `dead_letter_message`/`MaxDeliveryCountExceeded`, baca via `sub_queue=DEAD_LETTER`; validasi end-to-end KEDA bersama d1-03 | SRC-002, SRC-030, SRC-039, SRC-079–SRC-082 | Implementation sources verified | Draft |
| 21 | 3 | Develop event- and message-based AI solutions | Event-driven workflows via Event Grid: filters, custom events, retries | Azure Event Grid | d3-02 | Custom topic + publish array (curl key & Python Entra ID); 3 jenis filter (uji positif/negatif `data.color`); retry policy attempts/TTL + dead-letter ke storage container | SRC-002, SRC-031, SRC-083–SRC-089 | Implementation sources verified | Draft |
| 22 | 3 | Develop and implement Azure Functions | Bangun serverless APIs: triggers dan bindings | Azure Functions | d3-03 | Model Python v2 (decorator): HTTP trigger + route params, `@app.queue_output`/`@app.schedule`, blueprints; tabel binding resmi; 1 function = 1 trigger | SRC-002, SRC-032, SRC-090, SRC-092 | Implementation sources verified | Draft |
| 23 | 3 | Develop and implement Azure Functions | Konfigurasi dan deploy function apps | Azure Functions | d3-03 | `func init/new/start` → Flex Consumption passwordless (identity + `AzureWebJobsStorage__*`) → `func azure functionapp publish`; app settings via CLI + `os.getenv`; hosting plans | SRC-002, SRC-032, SRC-091–SRC-093 | Implementation sources verified | Draft |
...
| d3-01 | Service Bus namespace | Queue dan dead-letter handling dapat dilatih pada tier mana pun — DLQ adalah **subqueue bawaan** pada setiap queue dan topic subscription, bukan fitur tier (SRC-039). **Tier Basic tidak mendukung topics/subscriptions (SRC-079)** → lab memakai satu namespace **Standard** untuk seluruh latihan; hapus resource group setelah lab | Rendah–sedang |
| d3-02 | Event Grid custom topic | Topic dibayar per operasi; volume lab kecil (event >64 KB ditagih kelipatan 64 KB — SRC-083). Perhatian: lab juga men-deploy **Event Grid Viewer** (App Service plan + web app — SRC-087) dan storage account dead-letter — komponen inilah yang menagih terus; hapus resource group setelah lab | Rendah |
| d3-03 | Function app + storage account | **Flex Consumption** (Consumption legacy — Linux Consumption retire 30 Sep 2028, SRC-093); bayar per eksekusi + memori aktif; komponen yang menetap = storage account + Application Insights (dibuat otomatis, SRC-091) — hapus resource group setelah lab | Rendah–sedang |
...
| 2 | Azure Database for PostgreSQL + pgvector | [d2-02-azure-database-postgresql.md](d2-02-azure-database-postgresql.md) | **Draft — review di akhir** | Draft | 2026-07-15 |
| 2 | Azure Managed Redis | [d2-03-azure-managed-redis.md](d2-03-azure-managed-redis.md) | **Draft — review di akhir** | Draft | 2026-07-15 |
| 3 | Azure Service Bus | [d3-01-azure-service-bus.md](d3-01-azure-service-bus.md) | **Draft — review di akhir** | Draft | 2026-07-15 |
| 3 | Azure Event Grid | [d3-02-azure-event-grid.md](d3-02-azure-event-grid.md) | **Draft — review di akhir** | Draft | 2026-07-15 |
| 3 | Azure Functions | [d3-03-azure-functions.md](d3-03-azure-functions.md) | **Draft — review di akhir** | Draft | 2026-07-15 |
...
| SRC-071 | <https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-use-pgvector> | Docs artikel | Allowlist + `CREATE EXTENSION vector` (nama extension = vector); tipe `vector(n)`; INSERT/ON CONFLICT; operator `<->`/`<#>`/`<=>` + fungsi/agregat | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-072 | <https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/connect-python> | Docs quickstart | `psycopg`; Entra passwordless (scope `https://ossrdbms-aad.database.windows.net/.default`; token ±24 jam → refresh policy); sslmode=require; firewall; CRUD berparameter; Python ≥3.8 | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-073 | <https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-optimize-performance-pgvector> | Docs artikel | EXPLAIN (ANALYZE,VERBOSE,BUFFERS); IVFFlat (lists/probes + heuristik)/HNSW (m/ef_construction/ef_search)/DiskANN; **batas index 2000 dimensi**; access ops harus cocok operator; `<#>` utk vektor ternormalisasi; partial index (jebakan LIKE); partitioning + pruning | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-074 | <https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-pgbouncer> | Docs artikel | PgBouncer built-in 1.25.1; **port 6432**; pool_mode transaction (default); parameter; tidak tersedia di Burstable; Entra support; metrics/logs/admin console; batasan (single-threaded, restart saat failover) | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-075 | <https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-compute> | Docs artikel | Tier Burstable (CPU credit, "not recommended for production")/General Purpose (4 GiB/vCore)/Memory Optimized (6,75–9,5 GiB/vCore); storage 32 GiB–64 TiB; vCore naik/turun & storage naik dalam detik; tabel SKU + IOPS | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-076 | <https://learn.microsoft.com/en-us/azure/redis/overview> | Docs artikel | AMR = Redis Enterprise; Redis 7.4.x; cache-aside + expiration/eviction; 4 tier (Memory 8:1/Balanced 4:1/Compute 2:1/Flash) + tabel fitur (**search tidak di Flash**); HA/persistence/geo-replication; Entra semua tier; batas koneksi per SKU | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-077 | <https://learn.microsoft.com/en-us/azure/redis/overview-vector-similarity> | Docs artikel | RediSearch **hanya saat pembuatan**; syarat Enterprise clustering + NoEviction + tier non-Flash; index FLAT/HNSW; metrik L2/COSINE/IP; hash/JSON + metadata; KNN/range/hybrid search; semantic caching & LLM memory; tabel pemilihan layanan vector | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-078 | <https://learn.microsoft.com/en-us/azure/redis/python-get-started> | Docs quickstart | Packages `redis`/`redis-entraid`/`azure-identity`; **port AMR 10000** (vs 6380 Azure Cache for Redis); scope `https://redis.azure.com/.default`; Entra default + wajib tambah Redis user; contoh PING/SET/GET; Python ≥3.7 | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-079 | <https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-python-how-to-use-queues> | Docs quickstart | Packages `azure-servicebus`/`azure-identity`/`aiohttp`; penamaan namespace (6–50 karakter, mulai huruf, akhir huruf/angka, tanpa akhiran `-sb`/`-mgmt`); **tier Basic tidak mendukung topics**; roles Azure Service Bus Data Owner/Sender/Receiver + propagasi hingga 8 menit; kode async send/receive queue (`get_queue_sender/receiver`, `create_message_batch`, `receive_messages(max_wait_time, max_message_count)`, `complete_message`) | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-080 | <https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-queues-topics-subscriptions> | Docs artikel | Queue: FIFO, competing consumers, load leveling, temporal decoupling (pull model); topic/subscription = pub/sub 1:N, subscription seperti virtual queue; **receive-and-delete (at-most-once) vs peek-lock (dua tahap, at-least-once; + duplicate detection = exactly-once)**; rules/filters (default true filter) & actions atas properti pesan | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-081 | <https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-python-how-to-use-topics-subscriptions> | Docs quickstart | `get_topic_sender(topic_name)` / `get_subscription_receiver(topic_name, subscription_name, max_wait_time)`; pembuatan topic+subscriptions via Portal; halaman Subscription menampilkan hitungan pesan active vs dead-letter | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-082 | <https://learn.microsoft.com/en-us/python/api/overview/azure/servicebus-readme> | API/SDK README (azure-servicebus 7.14.3) | Python ≥3.9; `PEEK_LOCK` default vs `RECEIVE_AND_DELETE`; settlement `complete`/`abandon`/`dead_letter`/`defer` (+`receive_deferred_messages` via sequence number); **DLQ dibaca via `sub_queue=ServiceBusSubQueue.DEAD_LETTER`**; hasil `peek_messages()` tidak bisa di-settle; lock duration di entitas + `renew_message_lock`/`AutoLockRenewer`; sessions (`session_id`); scheduled messages; `az servicebus namespace create`; daftar exceptions (`MessageLockLostError`, `ServiceBusAuthorizationError`, `ServiceBusQuotaExceededError` — kuras entitas *atau subqueue-nya*, dll.); manajemen entitas = `azure-mgmt-servicebus`; klien tidak thread-safe; logging `azure.servicebus` | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-083 | <https://learn.microsoft.com/en-us/azure/event-grid/concepts> | Docs artikel | Konsep push delivery: events; **CloudEvents 1.0 format pilihan** (structured JSON; maks 1 MB; >64 KB ditagih kelipatan 64 KB); custom/system/partner topics; event subscriptions (+expiration); handlers (webhook retry sampai 200-OK; Storage Queue); security publish (key/SAS/Entra) & managed identity delivery → role di tujuan; **publish ke custom topic selalu array** | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-084 | <https://learn.microsoft.com/en-us/azure/event-grid/event-filtering> | Docs artikel | Tiga jenis filter (event types/subject begins-ends/advanced); desain subject bersegmen `/A/B/C`; operator advanced lengkap; string **case-insensitive**; key hilang → not matched (kecuali NumberNotIn/StringNotIn → matched); 1 filter multi-values = OR, antar filter = AND; batas **25 filter/25 values/512 karakter**; `enableAdvancedFilteringOnArrays`; key ber-titik tidak didukung | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-085 | <https://learn.microsoft.com/en-us/azure/event-grid/delivery-and-retry> | Docs artikel | **At-least-once; tanpa jaminan urutan**; tunggu 30 dtk; sukses hanya HTTP 200–204; jadwal backoff 10 dtk→…→tiap 12 jam maks 24 jam (tidak dapat dikonfigurasi); error non-retry (webhook 400/413/401/403) → dead-letter/dibuang; retry policy attempts 1–30 & TTL 1–1440 mnt (first-to-expire); delayed delivery/probation + tabel durasi; batching opsional; dead-letter: kondisi, delay 5 mnt, 4 jam unavailable → drop, `deadLetterReason`/`deliveryAttempts`/`lastDeliveryOutcome` (+tabel nilai) | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-086 | <https://learn.microsoft.com/en-us/python/api/overview/azure/eventgrid-readme> | API/SDK README (azure-eventgrid 4.22.0) | Python ≥3.8; Basic = **push only** vs Namespaces (pull+push; CloudEvents only); `EventGridPublisherClient(endpoint, credential).send(...)`; bentuk event (EventGridEvent/CloudEvent/dict); kirim list untuk performa; roles **Event Grid Data Sender/Receiver/Contributor**; endpoint `https://<topic>.<region>.eventgrid.azure.net/api/events`; schema topic ditetapkan saat pembuatan; `EventGridConsumerClient` hanya Namespaces; integrasi OpenTelemetry; logger `azure.eventgrid` | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-087 | <https://learn.microsoft.com/en-us/azure/event-grid/custom-event-quickstart> | Docs quickstart (CLI) | `az provider register Microsoft.EventGrid`; `az eventgrid topic create` (nama 3–50, DNS-unik); deploy **Event Grid Viewer** via `az deployment group create --template-uri`; `az eventgrid event-subscription create --source-resource-id --endpoint`; **subscription validation event** (handshake webhook); `topic show --query endpoint` / `topic key list --query key1`; contoh event Event Grid schema + `curl -H "aeg-sas-key"`; cleanup `az group delete` | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-088 | <https://learn.microsoft.com/en-us/azure/event-grid/how-to-filter-events> | Docs how-to | CLI filter: `--included-event-types`, `--subject-begins-with`/`--subject-ends-with`, `--advanced-filter data.color stringin blue red green`, `--expiration-date`; Portal Filters tab; pola uji positif (green terkirim) & negatif (yellow tidak) | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-089 | <https://learn.microsoft.com/en-us/azure/event-grid/manage-event-delivery> | Docs how-to | Dead-letter CLI `--deadletter-endpoint <storage-id>/blobServices/default/containers/<c>` (butuh storage+container); **nama blob memuat nama subscription HURUF BESAR** + path `YYYY/MM/DD/HH`; blob berisi array event; nonaktifkan dgn membuat ulang tanpa flag; managed identity perlu role tulis storage; retry policy CLI `--event-ttl` / `--max-delivery-attempts` (default 1440/30; jadwal tidak dapat dikonfigurasi; first-to-expire) | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-090 | <https://learn.microsoft.com/en-us/azure/azure-functions/functions-triggers-bindings> | Docs artikel | Trigger = **tepat satu** per function (tipe khusus input binding); bindings input/output deklaratif + contoh kombinasi; contoh Python v2 (`FunctionApp(http_auth_level)`, `@app.route`, `@app.queue_output`, `func.Out`); tabel binding runtime 4.x (HTTP/Timer/Queue/Service Bus/Event Grid/Event Hubs/Cosmos/Blob/SQL/Redis…); registrasi extension selain HTTP/Timer; **runtime 1.x end of support 14 Sep 2026** | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-091 | <https://learn.microsoft.com/en-us/azure/azure-functions/create-first-function-cli-python> | Docs quickstart (CLI) | `func init --worker-runtime python`; `func new --template "HTTP trigger" --authlevel function`; `func start` (access key tidak berlaku lokal); `local.settings.json` dikecualikan dari source control; storage `--allow-shared-key-access false` + user-assigned identity (Storage Blob Data Owner); `az functionapp create --flexconsumption-location … --deployment-storage-auth-type UserAssignedIdentity` (+App Insights otomatis; Monitoring Metrics Publisher); app settings `AzureWebJobsStorage__accountName/credential=managedidentity/clientId` + **hapus `AzureWebJobsStorage`**; `func azure functionapp publish` (remote build); `list-functions --show-keys`; `?code=`/`x-functions-key` | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-092 | <https://learn.microsoft.com/en-us/azure/azure-functions/functions-reference-python> | Docs developer guide | Model **v2 decorator (`function_app.py`) vs v1 (`function.json`, legacy)**; route params via `req.route_params` (bukan argumen); blueprints (`func.Blueprint` + `register_functions`); struktur project + tabel file kunci; `@app.schedule`/`@app.blob_input`/`@app.cosmos_db_input`/`@app.event_hub_output`; SDK type bindings hanya v2; env vars `local.settings.json` ↔ Application Settings + `os.getenv`; `requirements.txt`/remote build/ModuleNotFound; tabel mekanisme deployment (portal editor tanpa dependency eksternal); **retirement: v3 Linux Consumption 30 Sep 2026; plan Linux Consumption 30 Sep 2028**; logging → App Insights; OpenTelemetry; pytest; state global tidak dijamin persisten | Not shown | 2026-07-15 | Primary | — | OK |
| SRC-093 | <https://learn.microsoft.com/en-us/azure/azure-functions/functions-scale> | Docs artikel | Hosting options: **Flex Consumption** (GA; per-function scaling; maks 1.000 instance; 512/2048/4096 MB; always-ready; VNet) / Premium (prewarmed) / Dedicated / Container Apps / **Consumption legacy (Linux: Retired)**; **Python hanya Linux**; timeout per plan (Consumption 5/10 mnt; lainnya 30/unbounded); **HTTP wajib merespons ≤230 detik** (idle timeout Load Balancer); cold start + mitigasi; billing per plan; batasan pembuatan plan di resource group lama | Not shown | 2026-07-15 | Primary | — | OK |
...
| 2026-07-15 | Modul kesepuluh dibuat: **d3-03-azure-functions.md** (Draft; review di akhir) — **Domain 3 selesai draft**. 4 sumber baru diverifikasi (SRC-090–SRC-093); matrix #22–#23 → `Draft`/`Implementation sources verified`; guardrail biaya d3-03 diperbarui (Consumption = legacy → lab memakai **Flex Consumption**). Catatan verifikasi penting: 1 function = tepat 1 trigger; model Python v2 = decorator di `function_app.py` (route params via `req.route_params`); quickstart resmi kini passwordless (identity + `AzureWebJobsStorage__*`, hapus `AzureWebJobsStorage`); HTTP wajib merespons ≤230 dtk; retirement: runtime 1.x 14 Sep 2026, v3 Linux Consumption 30 Sep 2026, plan Linux Consumption 30 Sep 2028 | Eksekusi TAHAP 5 iterasi kesepuluh | SRC-090–SRC-093 |
| 2026-07-15 | Modul kesembilan dibuat: **d3-02-azure-event-grid.md** (Draft; review di akhir). 7 sumber baru diverifikasi (SRC-083–SRC-089); matrix #21 → `Draft`/`Implementation sources verified`; guardrail biaya d3-02 dipertegas (Viewer App Service plan + storage dead-letter = komponen yang menagih). Catatan verifikasi penting: publish ke custom topic **selalu array**; jadwal retry tidak dapat dikonfigurasi (hanya attempts 1–30 & TTL 1–1440 mnt, first-to-expire); webhook 400/413/401/403 **tidak di-retry** → tanpa dead-letter event dibuang; dead-letter default OFF + delay 5 menit; string advanced filter case-insensitive; antar filter = AND | Eksekusi TAHAP 5 iterasi kesembilan | SRC-083–SRC-089 |
| 2026-07-15 | Modul kedelapan dibuat: **d3-01-azure-service-bus.md** (Draft; review di akhir) — **mulai Domain 3**. 4 sumber baru diverifikasi (SRC-079–SRC-082); matrix #20 → `Draft`/`Implementation sources verified`; guardrail biaya d3-01 dipertegas (Basic tanpa topics → lab memakai Standard). Catatan verifikasi penting: `PEEK_LOCK` = mode default dan satu-satunya yang bisa settlement (complete/abandon/dead_letter/defer); DLQ dibaca sebagai receiver biasa dengan `sub_queue=ServiceBusSubQueue.DEAD_LETTER`; propagasi role RBAC hingga 8 menit; janji d1-03 (validasi end-to-end KEDA `azure-servicebus`) dieksekusi di lab §8.8 | Eksekusi TAHAP 5 iterasi kedelapan | SRC-079–SRC-082 |
| 2026-07-15 | Modul ketujuh dibuat: **d2-03-azure-managed-redis.md** (Draft; review di akhir) — **Domain 2 selesai draft**. 3 artikel baru diverifikasi (SRC-076–SRC-078); matrix #18–#19 → `Draft`/`Implementation sources verified`. Catatan verifikasi penting: modul RediSearch hanya bisa diaktifkan saat pembuatan instance (+ Enterprise clustering + NoEviction, tanpa Flash tier); port AMR = 10000 (pembeda dari Azure Cache for Redis 6380); sintaks FT.CREATE/FT.SEARCH tidak ada di halaman Microsoft yang diverifikasi → bagian kode vector ditandai Needs Review dgn rujukan docs tertaut | Eksekusi TAHAP 5 iterasi ketujuh | SRC-076–SRC-078 |
| 2026-07-15 | Modul keenam dibuat: **d2-02-azure-database-postgresql.md** (Draft; review di akhir). 5 artikel baru diverifikasi (SRC-071–SRC-075); coverage matrix #12–#17 → `Draft`/`Implementation sources verified`. Catatan verifikasi penting: nama extension = `vector` + wajib allowlist; batas index 2000 dimensi; access function harus cocok operator; PgBouncer built-in port 6432 tidak tersedia di tier Burstable | Eksekusi TAHAP 5 iterasi keenam | SRC-071–SRC-075 |
