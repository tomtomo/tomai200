...
> Status exam per audit terakhir: **Transitional/Unresolved** (lihat [Audit Terkini](#0-audit-terkini))
...
| Scheduling via Pearson VUE | Surface scheduling resmi (`schedule-through-pearson-vue?examUid=exam.AI-200`) ada, tetapi kontennya dinamis + sign-in gated sehingga schedulability AI-200 **tidak dapat diverifikasi langsung** | Halaman scheduling tidak mengembalikan konten statis; halaman "Register and schedule an exam" mendeskripsikan surface ini dan Pearson VUE sebagai provider | Sesuai protokol: scheduling bukan bukti GA | Low | 2026-07-15 | SRC-016, SRC-017 | Baseline tidak dapat dikonfirmasi ulang secara langsung |
| Bahasa exam | **BELUM DAPAT DIVERIFIKASI DARI SUMBER RESMI MICROSOFT** (bagian bahasa pada halaman exam dirender client-side) | Halaman sertifikasi tidak menampilkan bagian bahasa pada konten yang dapat diambil | Study guide hanya memuat pernyataan generik tentang lokalisasi exam dan tambahan 30 menit bila exam tidak tersedia dalam bahasa pilihan | Low | 2026-07-15 | SRC-001, SRC-002 | Baseline "English" tidak terkonfirmasi ulang → `Needs Review` |
| Assessment time AI-200 | **BELUM DAPAT DIVERIFIKASI** pada halaman AI-200 (dinamis). Kebijakan umum: associate role-based tanpa lab = 100 menit exam / 120 menit seat; yang dapat memuat lab = 120 / 140 | Tabel "Exam duration and exam experience" | Baseline menyebut "120 minutes" pada halaman AI-200 — tidak dapat dikonfirmasi ulang | Low (untuk angka AI-200 spesifik); High (untuk tabel kebijakan umum) | 2026-07-15 | SRC-007 | `Needs Review` |
...
| Study guide: last updated | **Not shown** — tidak ada tanggal/change log pada konten yang dapat diambil | Study guide AI-200 | Study guide AZ-204 (pembanding) menampilkan "Skills measured as of January 14, 2026" | Medium | 2026-07-15 | SRC-002, SRC-005 | `Needs Review` |
...
| Status | **Transitional/Unresolved** (tidak ada bukti resmi eksplisit yang menyelesaikan beta vs GA per tanggal cek) | SRC-001, SRC-015, SRC-020, SRC-021 |
...
| Bahasa exam | **BELUM DAPAT DIVERIFIKASI DARI SUMBER RESMI MICROSOFT** (bagian bahasa dirender client-side; baseline "English" tidak terkonfirmasi ulang). Study guide: bila exam tidak tersedia dalam bahasa pilihan, dapat meminta tambahan 30 menit | SRC-001, SRC-002 |
| Assessment time | Untuk AI-200 spesifik: **BELUM DAPAT DIVERIFIKASI** (dinamis). Kebijakan umum associate role-based: 100 menit (tanpa lab) atau 120 menit (dapat memuat lab) | SRC-007 |
...
| Scheduling | Surface resmi `schedule-through-pearson-vue?examUid=exam.AI-200` ada (dinamis, sign-in gated); schedulability tidak terverifikasi langsung dari environment audit | SRC-016, SRC-017 |
...
| # | Domain | Official skill heading | Skill (parafrase) | Layanan/fitur | File tujuan | Praktik wajib | Source ID | Coverage |
|---|---|---|---|---|---|---|---|---|
| 1 | 1 | Implement container application hosting | Build, store, version, dan manage container image di registry privat | Azure Container Registry | d1-01 | Build/tag/push/pull image; versioning dan manajemen repository | SRC-002, SRC-022 | Planned |
| 2 | 1 | Implement container application hosting | Build dan run image menggunakan ACR Tasks | ACR Tasks | d1-01 | `az acr build`, task otomatis | SRC-002, SRC-022 | Planned |
| 3 | 1 | Implement container application hosting | Deploy container ke App Service, termasuk environment variables dan secrets | Azure App Service (custom container) | d1-02 | Deploy image dari ACR; app settings & secret injection | SRC-002, SRC-023 | Planned |
| 4 | 1 | Implement container-orchestrated solutions | Deploy aplikasi ke Container Apps, termasuk konfigurasi environment dan revision management | Azure Container Apps | d1-03 | Buat environment; deploy; kelola revisions/traffic | SRC-002, SRC-024 | Planned |
| 5 | 1 | Implement container-orchestrated solutions | Terapkan event-driven scaling dengan KEDA di Container Apps | Container Apps + KEDA | d1-03 | Scale rule berbasis event source (mis. Service Bus queue) | SRC-002, SRC-025 | Planned |
| 6 | 1 | Implement container-orchestrated solutions | Deploy dan kelola aplikasi di AKS menggunakan manifest files | Azure Kubernetes Service | d1-04 | `kubectl apply` manifest Deployment/Service | SRC-002, SRC-026 | Planned |
| 7 | 1 | Implement container-orchestrated solutions | Monitor dan troubleshoot solusi di AKS dan Container Apps: logs, events, konektivitas end-to-end | AKS, Container Apps | d1-03 + d1-04 (cross-link) | Inspeksi logs/events/probes; uji konektivitas | SRC-002, SRC-024, SRC-026 | Planned |
| 8 | 2 | Develop AI solutions by using Azure Cosmos DB for NoSQL | Connect ke Cosmos DB for NoSQL via SDK dan jalankan query | Cosmos DB for NoSQL, Python SDK | d2-01 | CRUD + query via `azure-cosmos` | SRC-002, SRC-027 | Planned |
| 9 | 2 | Develop AI solutions by using Azure Cosmos DB for NoSQL | Optimasi performa query dan konsumsi RU via indexing policies dan consistency levels | Indexing policy, consistency, RU | d2-01 | Ubah indexing policy; ukur RU charge; uji consistency | SRC-002, SRC-027 | Planned |
| 10 | 2 | Develop AI solutions by using Azure Cosmos DB for NoSQL | Simpan/ambil embeddings dan jalankan vector similarity search untuk semantic retrieval | Vector search di Cosmos DB for NoSQL | d2-01 | Simpan embedding; vector query | SRC-002, SRC-027 | Planned |
| 11 | 2 | Develop AI solutions by using Azure Cosmos DB for NoSQL | Implement change feed processor untuk mendeteksi item baru/berubah | Change feed | d2-01 | Change feed processor Python | SRC-002, SRC-027 | Planned |
| 12 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Connect dan query Azure Database for PostgreSQL via SDK | Azure Database for PostgreSQL | d2-02 | Koneksi + query dari Python | SRC-002, SRC-028 | Planned |
| 13 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Modelkan schema dan strategi indexing: desain tabel dan pemilihan data type | Schema/index design | d2-02 | DDL tabel; pilih tipe data | SRC-002, SRC-028 | Planned |
| 14 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Strategi indexing: optimasi latency query dan pengurangan overhead compute pgvector | pgvector indexing | d2-02 | Bandingkan tipe index vector; ukur latency | SRC-002, SRC-028 | Planned |
| 15 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Konfigurasi compute, memory, dan storage untuk vector workloads | Sizing Flexible Server | d2-02 | Pilih SKU/konfigurasi untuk beban vector | SRC-002, SRC-028 | Planned |
| 16 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Vector similarity search: simpan embedding, semantic retrieval, pola RAG dengan metadata filter | pgvector + RAG | d2-02 | Similarity search + filter metadata (RAG) | SRC-002, SRC-028 | Planned |
| 17 | 2 | Develop AI solutions by using Azure Database for PostgreSQL | Optimasi koneksi untuk throughput dan latency | Connection pooling/optimization | d2-02 | Uji pooling; bandingkan latency | SRC-002, SRC-028 | Planned |
| 18 | 2 | Integrate Azure Managed Redis in AI solutions | Operasi data Azure Managed Redis: caching, expiration, invalidation | Azure Managed Redis | d2-03 | Cache-aside; TTL; invalidasi | SRC-002, SRC-029 | Planned |
| 19 | 2 | Integrate Azure Managed Redis in AI solutions | Vector indexing untuk similarity search | Vector search di Redis | d2-03 | Index vector + query kemiripan | SRC-002, SRC-029 | Planned |
| 20 | 3 | Develop event- and message-based AI solutions | Queue & proses operasi back-end via Service Bus: messages, topics, subscriptions, dead-letter handling | Azure Service Bus | d3-01 | Kirim/terima; topic+subscription; DLQ handling | SRC-002, SRC-030 | Planned |
| 21 | 3 | Develop event- and message-based AI solutions | Event-driven workflows via Event Grid: filters, custom events, retries | Azure Event Grid | d3-02 | Custom topic + event; filter; amati retry | SRC-002, SRC-031 | Planned |
| 22 | 3 | Develop and implement Azure Functions | Bangun serverless APIs: triggers dan bindings | Azure Functions | d3-03 | HTTP/queue trigger + binding | SRC-002, SRC-032 | Planned |
| 23 | 3 | Develop and implement Azure Functions | Konfigurasi dan deploy function apps | Azure Functions | d3-03 | Deploy + app settings | SRC-002, SRC-032 | Planned |
| 24 | 4 | Implement secure Azure solutions | Amankan secrets via Key Vault, termasuk rotation dan retrieval | Azure Key Vault | d4-01 | Simpan/ambil/rotasi secret via SDK | SRC-002, SRC-033 | Planned |
| 25 | 4 | Implement secure Azure solutions | Simpan dan ambil app configuration via App Configuration | Azure App Configuration | d4-02 | Key-value config + baca dari Python | SRC-002, SRC-034 | Planned |
| 26 | 4 | Monitor and troubleshoot Azure solutions | Trace distributed systems dengan OpenTelemetry SDKs | OpenTelemetry → Azure Monitor | d4-03 | Instrument traces/metrics/logs; kirim ke Application Insights | SRC-002, SRC-035 | Planned |
| 27 | 4 | Monitor and troubleshoot Azure solutions | Tulis KQL query untuk analisis logs dan metrics | KQL / Log Analytics | d4-03 | Filtering, aggregation, correlation, latency/error/dependency analysis | SRC-002, SRC-036, SRC-037 | Planned |
...
| d3-01 | Service Bus namespace | Tier terendah yang mendukung fitur lab (topics/DLQ butuh tier tertentu — verifikasi di dokumentasi) | Rendah–sedang |
...
|---|---|---|---|---|---|
| — | Induk: audit, peta, ledger | README.md | **Menunggu review (checkpoint TAHAP 4)** | — | 2026-07-15 |
| 1 | Azure Container Registry + ACR Tasks | d1-01-azure-container-registry.md | Belum dibuat | Planned | — |
| 1 | App Service custom container | d1-02-azure-app-service-container.md | Belum dibuat | Planned | — |
| 1 | Container Apps + KEDA | d1-03-azure-container-apps-keda.md | Belum dibuat | Planned | — |
| 1 | Azure Kubernetes Service | d1-04-azure-kubernetes-service.md | Belum dibuat | Planned | — |
| 2 | Cosmos DB for NoSQL | d2-01-cosmos-db-nosql.md | Belum dibuat | Planned | — |
...
| SRC-025 | <https://learn.microsoft.com/en-us/azure/container-apps/scale-app> | Docs artikel | "Azure Container Apps uses KEDA (Kubernetes Event-driven Autoscaling)"; scale rules & event sources | Not shown | 2026-07-15 | Primary | — | OK |
